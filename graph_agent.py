"""
NexusGraph AI — The Agentic Brain (LangGraph)
Implements the Think-Check-Execute loop:
  Router → Researcher → Critic → (loop if needed) → Answer
"""
import structlog
import json
...
Rules:
- Use "graph" when the question asks about relationships, collaborations, connections, "who worked with", "all X related to Y", etc.
...
async def router_node(state: AgentState) -> dict:
    """Route the query to vector, graph, or hybrid retrieval."""
...
async def researcher_node(state: AgentState) -> dict:
    """Fetch context from vector store and/or graph DB based on routing decision."""
    step_num = len(state["agent_trace"]) + 1
            ...
            if vector_results:
                ...
                for i, r in enumerate(vector_results[:5], 1):
                    context_parts.append(
                        f"[{i}] (score={r.score:.3f}) {r.content[:400]}"
                    )
        except Exception as e:
        ...
        try:
            if state["extracted_entities"]:
                ...
                if graph_result.nodes:
                    ...
                    context_parts.append(f"Found {len(graph_result.nodes)} related documents via graph traversal:")
                    for node in graph_result.nodes[:8]:
                        title = node.properties.get("title", node.id)
                        context_parts.append(f"  • {title}")
                    if graph_result.relationships:
                        context_parts.append(f"Via {len(graph_result.relationships)} relationship paths:")
...
async def critic_node(state: AgentState) -> dict:
    """Evaluate answer quality; flag for retry if confidence is low."""
    step_num = len(state["agent_trace"]) + 1
    logger.info("agent.critic", retry_count=state.get("retry_count", 0))

    llm = get_llm(temperature=0.1)

    # First generate a draft answer
    answer_messages = [
...
async def run_research_query(question: str, max_hops: int = 2,
    ...
    """Execute the full Think-Check-Execute agent loop."""
    import time
    if query_id is None:
